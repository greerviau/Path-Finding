...
public class PathFinding {
	...
	private void initialize() {	//INITIALIZE THE GUI ELEMENTS
		frame = new JFrame();
		frame.setIconImage(Toolkit.getDefaultToolkit().getImage(PathFinding.class.getResource("/image/icon.png")));
		...
		canvas.addMouseListener(new MouseAdapter() {	//MOUSE LISTENER FOR THE GRID
			@Override
			public void	mousePressed(MouseEvent e) {
				...
				try {
					...
					int y = e.getY()/CSIZE;
					Node current = map[x][y];
					switch(tool) {	//TOOLBOX ITEM CURRENTLY SELECTED
						case 0:	//START NODE
						...
						case 1:	//FINISH NODE
							...
							break;
						default:	//WALL OR EMPTY NODE
							current.setType(tool);	//SET NODE CLICKED TO BE EITHER EMPTY OR WALL
							break;
					}
					Update();
